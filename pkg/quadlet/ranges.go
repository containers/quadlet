...
/* Keep track of a list of ranges of uint32, used to manage Uid/Gid ranges for mapping */
...
func (r *Ranges) Remove(start, length uint32) {
	...
	for i := 0; i < len(r.Ranges); i++ {
		...
		if end > currentStart && start < currentEnd {
			...
			if remainingAtStart == 0 && remainingAtEnd == 0 {
				// Remove whole range
				copy(r.Ranges[i:], r.Ranges[i+1:])
				r.Ranges = r.Ranges[:len(r.Ranges)-1]
				i-- // undo loop iter
			} else if remainingAtStart != 0 && remainingAtEnd != 0 {
				// Range is split
