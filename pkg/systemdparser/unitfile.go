...
/* Code to parse, modify and re-emit system unit files */
...
// Last instance of the key wins
// Can have trailing space
// Raw == contains  continuations
...
// Can have trailing space
// Raw == contains line continuations
...
// Unescapes exec-like arguments for all instances of the key
func (f *UnitFile) LookupAllArgs(groupName string, key string) []string {
	res := make([]string, 0)
	argsv := f.LookupAll(groupName, key)
	for _, args_s := range argsv {
		args, err := splitString(args_s, WhitespaceSeparators, SplitRelax|SplitUnquote|SplitCUnescape)
		if err == nil {
			res = append(res, args...)
		}
	}
...
// Unescapes exec-like arguments for the last instance of the key
func (f *UnitFile) LookupLastArgs(groupName string, key string) ([]string, bool) {
	exec_key, ok := f.LookupLast(groupName, "Exec")
	if ok {
		exec_args, err := splitString(exec_key, WhitespaceSeparators, SplitRelax|SplitUnquote|SplitCUnescape)
		if err == nil {
			return exec_args, true
		}
	}
	return nil, false
}
...
func (f *UnitFile) LookupAllKeyVal(groupName string, key string) map[string]string {
	...
	all_keyvals := f.LookupAll(groupName, key)
	for _, keyvals := range all_keyvals {
