...
func cUnescapeOne(p string, acceptNul bool) (count int, ret rune, eightBit bool) {
	count = 1
	eightBit = false
...
func extractFirstWord(in string, separators string, flags SplitFlags) (string, string, bool, error) {
	...
	for c = in[0]; ; c = nextChar() {
		...
		} else if strings.ContainsRune(separators, rune(c)) {
	...
	for ; ; c = nextChar() {
		if backslash {
			...
			for ; ; c = nextChar() {
				...
				} else if c == '\\' && !(flags&SplitRetainEscape != 0) {
					backslash = true
					break
				}
			...
			for ; ; c = nextChar() {
				if c == 0 {
					goto finishForceTerminate
				} else if (c == '\'' || c == '"') && (flags&(SplitKeepQuote|SplitUnquote) != 0) {
					quote = c
					if flags&SplitUnquote != 0 {
						break
					}
